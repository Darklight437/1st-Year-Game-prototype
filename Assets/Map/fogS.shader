...
Shader "Shader Forge/fogS" {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _node_527; uniform float4 _node_527_ST;
            uniform sampler2D _node_3342; uniform float4 _node_3342_ST;
            uniform float _node_9343;
            uniform float _Xpan;
            uniform float _Ypan;
            struct VertexInput {
...
/////// Diffuse:
                ...
                float4 node_7793 = tex2D(_node_527,TRANSFORM_TEX(node_3275, _node_527));
                float3 diffuseColor = node_7793.rgb;
                float3 diffuse = (directDiffuse + indirectDiffuse) * diffuseColor;
/// Final Color:
                float3 finalColor = diffuse;
                float4 node_9744 = tex2D(_node_3342,TRANSFORM_TEX(i.uv0, _node_3342));
                return fixed4(finalColor,(_node_9343*(1.0 - node_9744.r)));
            }
            ENDCG
        }
        Pass {
...
/// Final Color:
                float3 finalColor = diffuse;
                float4 node_9744 = tex2D(_node_3342,TRANSFORM_TEX(i.uv0, _node_3342));
                return fixed4(finalColor * (_node_9343*(1.0 - node_9744.r)),0);
            }
            ENDCG
        }
    }
